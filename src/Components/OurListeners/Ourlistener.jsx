    ...
    <div className="bg-[#111111] flex flex-col  justify-center items-center  px  ">
      ...
      <div className="cards pb-20 flex flex-wrap justify-center gap-[24px]">
        {listeners.map((listener, index) => (
          <div
            key={index}
            className={`card rounded-[16px] bg-[#1f1f1f] py-[40px]   ${
              isLgScreen ? "px-[24px] w-[411px]" : "px-3 w-[300px]"
            }  h-[560px] `}
          >
            <div className="upper gap-[28px] flex">
              <div className="left flex flex-col justify-center items-center">
                ...
                <div
                  className={` ${
                    isLgScreen
                      ? "py-2 w-[125px] text-[18px]"
                      : "py-1 w-[90px] text-[14px]"
                  } mt-[-12px]  text-center font-normal bg-[#464646] text-[#d9d9d9]  rounded-[40px]`}
                >
                  F - {listener.age}
                </div>
              </div>
              <div className="right gap-[16px]">
                <span className="text-[24px] text-[#d9d9d9]">
                  {listener?.name}
                </span>
                <div className="star flex items-center py-[12px] gap-[12px]">
                  <img src="caringlistenerstar.svg" alt="Star" />
                  <span className="text-[18px] font-normal text-[#808080]">
                    {listener.rating}({listener.reviews})
                  </span>
                </div>
            ...
            <div className="below flex justify-between items-center">
              <span className="text-[20px] font-medium text-[#d9d9d9]">
                Exp-{listener.experience}
              </span>
              <div
                onClick={() => handleViewListenerDesc({ data: listener })}
                className={`text-[#111111] ${
                  isLgScreen
                    ? "text-[18px] px-[2px] py-[2px]"
                    : " text-[15px] px-[2px]"
                } bg-[#d9d9d9] hover:bg-[#d9d9d9] hover:shadow-[0_0_28px_13px_rgba(217,217,217,0.2)] shadow-[0_0_17px_7px_rgba(217,217,217,0.2)] hover:text-[#111111] font-semibold  duration-300 cursor-pointer rounded-full bg-gradient-to-t from-transparent to-[#d9d9d9]`}
              >
                <button
                 
                  className={` ${
                    isLgScreen ? "px-6 py-3" : "px-4 py-3"
                  }  rounded-full`}
